...
export default function StatTable({ rows, keyLabel, currency, emptyText = 'No data yet.' }) {
  if (!rows || rows.length === 0) {
    return <div className="px-1 py-6 text-center text-sm text-white/40">{emptyText}</div>;
  }
  return (
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.key} className="border-b border-white/5 last:border-0">
              <td className="py-2 pr-3 font-medium text-white/90">{r.key}</td>
              <td className="py-2 px-2 text-right font-mono tabular-nums text-white/60">{r.sessions}</td>
              <td className="py-2 px-2 text-right font-mono tabular-nums text-white/60">
